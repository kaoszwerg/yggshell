...
describe("api", () => {
  ...
  describe("updateSettings", () => {
    ...
    it("carries the terminal font as a plain name", async () => {
      // Not validated against a list, because there is no list: a WebView cannot enumerate fonts, so
      // the backend stores what it is given and the CSS stack falls back if it is not installed.
      await api.updateSettings({ terminalFont: "MesloLGS NF" });
      expect(mockInvoke).toHaveBeenCalledWith("update_settings", {
        uiScale: null,
        terminalFontSize: null,
        toolFontSize: null,
