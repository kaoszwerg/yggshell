...
> Copy this file into whatever a project uses to brief its agents. It names no stack, needs no
> tooling and cites nothing outside itself, so it works alone. If the receiving project has a
> governance system with front-matter, add the fields that system expects — they belong to the system
> that reads them, not to the rule.
...
says which** (measured, in a real project: the target came from a constant imported by the test
config, never from the command line).
...
mean?"* — a question a project should be able to answer anyway. **It is project-owned**: nothing
delivers it, nothing overwrites it.
...
**Declaring is not renaming.** An existing project fills this in without moving a single script,
which is why adoption costs an afternoon rather than a quarter.
...
deliberately not enforced.** Forcing it was built and measured: a gate on file edits missed the
ordinary route entirely, because the agent wrote its files through the shell instead — and where it
did fire, it produced a task restating the tool call it had just blocked. A plan-shaped artefact that
reads as information and is none is worse than an honest gap. What works is this rule being in
context and the work being large enough that a list pays for itself, which is a judgement the agent
usually gets right.
...
## Why this is a rule and not (only) a check

The checkable parts should be checked: that `work-levels.json` parses, that every entry uses a valid
act/refinement/target, and that a non-`local` target carries `reaches`. A script for exactly that
ships beside this file.

What no checker can do is decide whether `verify/integration@dev` is the **truthful** label. A run
named `unit` that quietly reaches a database is a lie the file cannot detect — only the person
writing it can, at the moment they write it. That is why the rule is addressed to them, and why it
belongs in whatever a project loads at the *start* of a task rather than on demand: **the failure
emits no vocabulary.** Somebody naming a script is not thinking about a naming convention and will
